...
The `## Notes for tailoring lessons` section in `02-diego-profile.md` is itself an output of an earlier step (the `write-student` agent) and demonstrates the system's "operational tailoring" contract — every bullet must name a specific interest AND a specific instructional move, not vague guidance like "use his interests." Two bullets later, when the tailoring step picks `GP38-2` and `SD40-2` as anchors, it is pulling those locomotive class names forward from this profile section, which in turn pulled them forward from the teacher's free-text dump.

## Honest read on quality

The tailoring is *good but not great*. Every problem now opens with a Diego-specific anchor and the concept mapping is preserved — that is a clear win over a generic master-everywhere baseline. But the anchors are used as *settings* rather than as the *analogical mechanism*; *"In Mark Felton, a train engineer needs to ensure the train's brakes…"* puts Mark Felton in front of the master problem without using his content as the bridge to photosynthesis. The system's theoretical ceiling is operational tailoring where the anchor's mechanics explain the concept (the kelp-forests-in-Ponyo example in `skills/knowledge/teaching-basics.md` is the target shape). The 2 B-parameter Gemma 3n model can substitute reliably but cannot reach for genuine cross-domain analogy on every problem yet; `docs/tailor-decomposition.md` specifies the next decomposition (one micro-agent per anchor) that would close that gap. We chose to ship the current state because the divergence validator and the concept-mapping validator together guarantee that nothing sub-par appears as a PDF — when the model fails to substitute, the validator refuses the file and the user re-runs the flow with a clear failure message rather than receiving silent garbage.
